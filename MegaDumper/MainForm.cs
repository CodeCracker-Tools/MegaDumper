...
namespace Mega_Dumper
{
    ...
    public partial class MainForm : Form
    {
        ...
        static extern int NtQueryInformationProcess(IntPtr processHandle,
            int processInformationClass, ref PROCESS_BASIC_INFORMATION processInformation,
                    ...
                    do
                    {
                        ...
                        if (!isThere)
                        {
                                    ...
                                    try
                                    {
                                        IntPtr hProcess =
                                            OpenProcess(
                                                ...
                                                PROCESS_VM_READ, 0, procEntry.th32ProcessID);
                                        if (hProcess != IntPtr.Zero)
                                        {
                                            PROCESS_BASIC_INFORMATION pbi = new PROCESS_BASIC_INFORMATION();
                                            int bytesWritten;
                                            int result = NtQueryInformationProcess(hProcess, 0, ref pbi,
                                                (uint) Marshal.SizeOf(pbi), out bytesWritten);
                                            if (result >= 0) // == 0 is OK
                                            {
                                                byte[] peb = new byte[472];
                                                uint BytesRead = 0;
                                                bool isok = ReadProcessMemory(hProcess, (IntPtr) (pbi.PebBaseAddress),
                                                    peb, (uint) (peb.Length), ref BytesRead);
                                                if (isok)
                                                {
                                                    ...
                                                    isok = ReadProcessMemory(hProcess, AProcessParameters,
                                                        ProcessParameters, (uint) (ProcessParameters.Length),
                                                        ref BytesRead);
                                                    if (isok)
                                                    {
                                                        int aCurrentDirectory =
                                                            BitConverter.ToInt32(ProcessParameters, 040);
                                                        byte[] Forread = new byte[2];
                                                        int size = 0;

                                                        do
                                                        {
                                                            isok = ReadProcessMemory(hProcess,
                                                                (IntPtr) (aCurrentDirectory + size), Forread, 2,
                                                                ref BytesRead);
                                                            size = size + 2;
                                                        } while (isok && Forread[0] != 0);
                                                        size = size - 2;
                                                        byte[] CurrentDirectory = new byte[size];
                                                        isok = ReadProcessMemory(hProcess, (IntPtr) (aCurrentDirectory),
                                                            CurrentDirectory, (uint) size, ref BytesRead);
                                                        newname = System.Text.Encoding.Unicode.GetString(
                                                            CurrentDirectory);
                                                        if (newname.Length >= 3)
                                                        {
                            ...
                            string[] prcdetails = new string[]
                                {processname, procEntry.th32ProcessID.ToString(), "", isnet, directoryName};
            ...
            try
            {
                ...
                if (Process32First(handleToSnapshot, ref procEntry))
                {
                    do
                    {
                                ...
                                try
                                {
                                    ...
                                    if (hProcess != IntPtr.Zero)
                                    {
                                        ...
                                        if (result >= 0) // == 0 is OK
                                        {
                                            byte[] peb = new byte[472];
                                            uint BytesRead = 0;
                                            bool isok = ReadProcessMemory(hProcess, (IntPtr) (pbi.PebBaseAddress), peb,
                        ...
                        string[] prcdetails = new string[]
                            {processname, procEntry.th32ProcessID.ToString(), statut, isnet, directoryName};
                        ListViewItem proc = new ListViewItem(prcdetails);
                        lvprocesslist.Items.Add(proc);
                    } while (Process32Next(handleToSnapshot, ref procEntry));
                }
        ...
        private static extern int AdjustTokenPrivileges(int tokenhandle, int disableprivs,
            ref TOKEN_PRIVILEGES Newstate, int bufferlength, int PreivousState, int Returnlength);

        [DllImport("advapi32.dll", CharSet = CharSet.Auto, SetLastError = true)]
        private static extern int GetSecurityInfo(int HANDLE, int SE_OBJECT_TYPE, int SECURITY_INFORMATION,
            int psidOwner, int psidGroup, out IntPtr pDACL, IntPtr pSACL, out IntPtr pSecurityDescriptor);

        [DllImport("advapi32.dll", CharSet = CharSet.Auto, SetLastError = true)]
        private static extern int SetSecurityInfo(int HANDLE, int SE_OBJECT_TYPE, int SECURITY_INFORMATION,
            int psidOwner, int psidGroup, IntPtr pDACL, IntPtr pSACL);
        ...
        public bool FixImportandEntryPoint(int dumpVA, byte[] Dump)
        {
                        ...
                        if (ismscoree)
                        {
                            ...
                            if (OriginalFirstThunkfo > 0)
                            {
                                ...
                                if (ThunkDatafo > 0)
                                {
                                    ...
                                    for (int i = 0; i < mscoreeAscii.Length; i++)
                                    {
                                        if (Dump[ThunkDatafo + 2 + i] != CorExeMain[i] &&
                                            Dump[ThunkDatafo + 2 + i] != CorDllMain[i])
                                        {
                                            ismscoree = false;
                                            break;
                                        }
                                    ...
                                    {
                                        ThunkToFix =
                                            BitConverter.ToInt32(Dump, impdiroffset + current + 16); // FirstThunk;
                                        break;
                                    }
                                }
                            }
            ...
            if (EntryPoint <= 0 || RVA2Offset(Dump, EntryPoint) < 0)
            {
                ...
                {
                    if (Dump[i + 0] == 0x0FF && Dump[i + 1] == 0x025 && Dump[i + 2] == ThunkToFixbytes[0] &&
                        Dump[i + 3] == ThunkToFixbytes[1] && Dump[i + 4] == ThunkToFixbytes[2] &&
                        Dump[i + 5] == ThunkToFixbytes[3])
                    {
        ...
        unsafe void DumpProcess()
        {
            ...
            if (intselectedindex != -1)
            {
                ...
                IntPtr hProcess =
                    OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_VM_READ,
                        0, processId);
                ...
                if (hProcess == IntPtr.Zero)
                {
                    ...
                    GetSecurityInfo((int) Process.GetCurrentProcess().Handle, /*SE_KERNEL_OBJECT*/
                        6, /*DACL_SECURITY_INFORMATION*/ 4, 0, 0, out pDACL, IntPtr.Zero, out pSecDesc);
                    hProcess = OpenProcess(0x40000, 0, processId);
                    SetSecurityInfo((int) hProcess, /*SE_KERNEL_OBJECT*/ 6, /*DACL_SECURITY_INFORMATION*/
                        4 | /*UNPROTECTED_DACL_SECURITY_INFORMATION*/ 0x20000000, 0, 0, pDACL, IntPtr.Zero);
                    CloseHandle(hProcess);
                    hProcess = OpenProcess(
                        PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_VM_READ, 0,
                        processId);
                }
                ...
                if (hProcess != IntPtr.Zero)
                {
                            ...
                            for (int k = 0; k < onepage.Length - 2; k++)
                            {
                                if (onepage[k] == 0x4D && onepage[k + 1] == 0x5A)
                                {
                                    if (ReadProcessMemory(hProcess, (uint) (j + k + 0x03C), infokeep, 4, ref BytesRead))
                                    {
                                        ...
                                        {
                                            if (ReadProcessMemory(hProcess, (uint) (j + k + PEOffset), infokeep, 2,
                                                ref BytesRead))
                                            {
                                                if (infokeep[0] == 0x050 && infokeep[1] == 0x045)
                                                {
                                                    long NetMetadata = 0;
                                                    if (ReadProcessMemory(hProcess, (uint) (j + k + PEOffset + 0x0E8),
                                                        infokeep, 8, ref BytesRead))
                                                    ...
                                                    if (dumpNativeToolStripMenuItem.Checked || NetMetadata != 0)
                                                    {
                                                        ...
                                                        if (ReadProcessMemory(hProcess, (uint) (j + k), PeHeader,
                                                            pagesize, ref BytesRead))
                                                        {
                                                            int nrofsection =
                                                                (int) BitConverter.ToInt16(PeHeader, PEOffset + 0x06);
                                                            if (nrofsection > 0)
                                                            {
                                                                bool isNetFile = true;
                                                                string dumpdir = "";
                                                                ...
                                                                int sectionalignment =
                                                                    BitConverter.ToInt32(PeHeader, PEOffset + 0x038);
                                                                int filealignment =
                                                                    BitConverter.ToInt32(PeHeader, PEOffset + 0x03C);
                                                                short sizeofoptionalheader =
                                                                    BitConverter.ToInt16(PeHeader, PEOffset + 0x014);

                                                                bool IsDll = false;
                                                                if ((PeHeader[PEOffset + 0x017] & 32) != 0)
                                                                    IsDll = true;
                                                                IntPtr pointer = IntPtr.Zero;
                                                                image_section_header[] sections =
                                                                    new image_section_header[nrofsection];
                                                                uint ptr = (uint) (j + k + PEOffset) +
                                                                           (uint) sizeofoptionalheader + 4 +
                                                                           (uint) Marshal.SizeOf(
                                                                ...
                                                                for (int i = 0; i < nrofsection; i++)
                                                                {
                                                                    ...
                                                                    ReadProcessMemory(hProcess, ptr, datakeeper,
                                                                        (uint) datakeeper.Length, ref BytesRead);
                                                                    ...
                                                                    sections[i] =
                                                                        (image_section_header) Marshal.PtrToStructure(
                                                                            pointer, typeof(image_section_header));
                                                                    ptr = ptr + (uint) Marshal.SizeOf(
                                                                              typeof(image_section_header));
                                                                }
                                                                ...
                                                                int totalrawsize = 0;
                                                                int rawsizeoflast = sections[nrofsection - 1]
                                                                    .size_of_raw_data;
                                                                int rawaddressoflast = sections[nrofsection - 1]
                                                                    .pointer_to_raw_data;
                                                                ...
                                                                int actualsizeofimage =
                                                                    BitConverter.ToInt32(PeHeader, PEOffset + 0x050);
                                                                int sizeofimage = actualsizeofimage;
                                                                int calculatedimagesize = BitConverter.ToInt32(PeHeader,
                                                                    PEOffset + 0x0F8 + 012);
                                                                int rawsize,
                                                                    rawAddress,
                                                                    virtualsize,
                                                                ...
                                                                for (int i = 0; i < nrofsection; i++)
                                                                {
                                                                    ...
                                                                    calculatedimagesize =
                                                                        calculatedimagesize + virtualsize + toadd;
                                                                }
                                                                ...
                                                                if (totalrawsize != 0)
                                                                {
                                                                    try
                                                                    {
                                                                        ...
                                                                        isok = ReadProcessMemory(hProcess,
                                                                            (uint) (j + k), rawdump,
                                                                            (uint) rawdump.Length, ref BytesRead);
                                                                        if (isok)
                                                                        {
                                                                            CreateTheFile1:
                                                                            ...
                                                                            filename = dumpdir + "\\rawdump_" +
                                                                                       (j + k).ToString("X8");
                                                                            if (File.Exists(filename))
                                                                                filename = dumpdir + "\\rawdump" +
                                                                            ...
                                                                            catch
                                                                            {
                                                                                ...
                                                                                browse.ShowNewFolderButton = false;
                                                                                browse.Description =
                                                                                    "Failed to create the file - select a new location:";
                                                                ...
                                                                for (int l = 0; l < nrofsection; l++)
                                                                {
                                                                    ...
                                                                    if (calcrawsize != 0 && rawsize != calcrawsize &&
                                                                        rawsize != virtualsize
                                                                        || rawAddress < 0)
                                                                    {
                                                                        // if raw size is bad:
                                                                        ...
                                                                        BinaryWriter writer =
                                                                            new BinaryWriter(
                                                                                new MemoryStream(virtualdump));
                                                                        writer.BaseStream.Position =
                                                                            PEOffset + 0x0F8 + 0x28 * l + 16;
                                                                        writer.Write(virtualsize);
                                                                        writer.BaseStream.Position =
                                                                            PEOffset + 0x0F8 + 0x28 * l + 20;
                                                                    ...
                                                                    isok = ReadProcessMemory(hProcess,
                                                                        (uint) (j + k + virtualAddress), csection,
                                                                        (uint) rawsize, ref BytesRead);
                                                                    if (!isok || BytesRead != rawsize)
                                                                    {
                                                                            ...
                                                                            c = c + (int) pagesize)
                                                                        {
                                                                            ...
                                                                            {
                                                                                isok = ReadProcessMemory(hProcess,
                                                                                    (uint) (j + k + virtualAddress + c),
                                                                                    currentpage, (uint) pagesize,
                                                                                    ref BytesRead);
                                                                            }
                                                                            catch
                                                                            {
                                                                    ...
                                                                    try
                                                                    {
                                                                        Array.Copy(csection, 0, virtualdump, rawAddress,
                                                                            rightsize);
                                                                    }
                                                                    catch
                                                                    {
                                                                ...
                                                                filename = dumpdir + "\\vdump_" +
                                                                           (j + k).ToString("X8");
                                                                if (File.Exists(filename))
                                                                    filename = dumpdir + "\\vdump" +
                                                                               CurrentCount.ToString() + "_" +
                                                                               (j + k).ToString("X8");
                                                                ...
                                                                if (fout != null)
                                                                {
                                                                    if (rightrawsize > virtualdump.Length)
                                                                        rightrawsize = virtualdump.Length;
                        ...
                        if (Directory.Exists(ddirs.dumps))
                        {
                            ...
                            foreach (FileInfo fi in rgFiles)
                            {
                                ...
                                if (info.CompanyName != null &&
                                    info.CompanyName.ToLower().Contains("microsoft corporation")
                                ...
                                if (info.OriginalFilename != null && info.OriginalFilename != "")
                                {
                                    ...
                                    if (File.Exists(Newfilename))
                                    {
                                        ...
                                        do
                                        {
                                            Newfilename =
                                                placedir + "\\" +
                                                Path.GetFileNameWithoutExtension(info.OriginalFilename)
                                ...
                                else
                                {
                                    ...
                                    if (File.Exists(Newfilename))
                                    {
                                        ...
                                        {
                                            Newfilename =
                                                ddirs.unknowndirname + "\\" + Path.GetFileNameWithoutExtension(fi.Name)
                                                + "(" + count.ToString() + ")" + extension;
                    ...
                    CurrentCount--;
                    MessageBox.Show(CurrentCount.ToString() + " files dumped in directory " + ddirs.dumps, "Success!",
        ...
        void KillProcessToolStripMenuItemClick(object sender, EventArgs e)
        {
            ...
            if (intselectedindex != -1)
            {
                ...
                IntPtr hProcess =
                    OpenProcess(
                        PROCESS_TERMINATE | PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_WRITE |
        ...
        void ViewHeapToolStripMenuItemClick(object sender, EventArgs e)
        {
            if (lvprocesslist.SelectedIndices.Count > 0)
            {
                ...
                if (strprname != "")
                {
                    ...
                    {
                        MessageBox.Show("Can't enumerate heap for MegaDumper itself!", "Error", MessageBoxButtons.OK,
